...
# M3 is used for error correction (Use 'pip install mthree' to install)
import mthree
...
# Source: https://nonhermitian.org/posts/2021/2021-10-07-vqe_program.html
...
def main(backend, user_messenger,
    ...
    full_circs = [ansatz_circuit.compose(mcirc).measure_all(
        inplace=False) for mcirc in meas_circs]
...
def fmin_spsa(func, x0, args=(), maxiter=100,
    ...
    for kk in range(maxiter):
        ...
        grad = (func(x + ck*deltak, *args) -
                func(x - ck*deltak, *args))/(2*ck*deltak)
        x -= ak*grad
